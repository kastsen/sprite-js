(function(glob){
     ...
     * @param {Number} numCols - The number of columns in the sprite sheet
     * @param {Number} numRows - The number of rows in the sprite sheet
     * @returns {Array}
     */
    var convertToFramePoints = function(frameObject, numFrames, numCols, numRows){
            var result = [];
            for (var i = 0; i < numFrames; i += 1) {
                var x = (i % numCols) * frameObject.width;
                var y = Math.floor(i / numCols) * frameObject.height;
                result.push([x, y, frameObject.width, frameObject.height]);
            }
            return result;
        },
        /*
    ...
    var Sprite = function(el, frames, options){
        ...
        var numCols = Math.floor(options.imageWidth / frames.width);
        var numRows = Math.floor(options.imageHeight / frames.height);
        this.numFrames = frames.length ?
            frames.length :
            numCols * numRows;
        framePoints = !isArray(frames) ? convertToFramePoints(frames, this.numFrames, numCols, numRows) : frames;
